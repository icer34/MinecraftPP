...
//* indexed in the same order as the Direction enum (directions.h), so
// CUBE_FACE_VERTICES[(size_t)dir] works.
        ...
        for (int y = 0; y < Chunk::HEIGHT; y++)
        {
            for (int z = 0; z < Chunk::SIZE; z++)
            {
                ...
                for (Direction dir : ALL_DIRECTIONS)
                {
                    ...
                    if (block.isSolid)
                        addFace(block, dir, glm::ivec3(x, y, z), solidVert, solidIdx);
                    if (block.isLiquid)
                        addFace(block, dir, glm::ivec3(x, y, z), waterVert, waterIdx);
                }
            }
        }
    }
                          ...
                          std::vector<unsigned int> &indices)
{
    ...
    {
        const UVRect &rect = face.layers[layerIdx].uv;
        unsigned int addedFaces =
            static_cast<unsigned int>(vert.size() / 8); //* 8 = number of floats per vertex

        for (int i = 0; i < 4; i++)
        {
            // position
            glm::vec3 pos = glm::vec3(localPos) +
                            facePos[i]; // shift the vertex pos by the position in the chunk
