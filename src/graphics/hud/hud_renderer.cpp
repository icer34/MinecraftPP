...
void HudRenderer::loadIconAtlas()
{
    ...
    for (const auto &entry : std::filesystem::recursive_directory_iterator("assets/textures/hud"))
    {
    ...
    m_iconAtlasTexture = Texture(nullptr, ATLAS_WIDTH, atlasHeight, GL_RGBA8, GL_RGBA);
    for (auto &icon : iconData)
    {
        ...
        m_iconUV.insert({icon.name,
                         UVRect{(float)icon.x / ATLAS_WIDTH,
                                ...
                                (float)icon.y / atlasHeight,
                                (float)(icon.y + icon.h) / atlasHeight}});
    }
...
void HudRenderer::drawText(const std::string &text, vec2 pos, float scale, vec4 color)
{
    ...
    for (size_t i = 0; i < text.length(); i++)
    {
        ...
        m_textVertData.push_back(HudVertex{
